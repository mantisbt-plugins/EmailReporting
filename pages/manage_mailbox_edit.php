...
elseif ( $f_mailbox_action === 'test' && $f_select_mailbox >= 0 )
{
	...
	$t_result = ERP_process_all_mails( $t_mailbox, TRUE );

	$t_mailbox_hostname = ERP_prepare_mailbox_hostname( $t_mailbox );

	$t_is_custom_error = ( is_array( $t_result ) && $t_result[ 'ERROR_TYPE' ] === 'NON-PEAR-ERROR' );
...
<?php
		...
		echo plugin_lang_get( 'mailbox_description' ) . ': ' . $t_mailbox[ 'mailbox_description' ] . '<br />';
		echo plugin_lang_get( 'mailbox_type' ) . ': ' . $t_mailbox[ 'mailbox_type' ] . '<br />';
		echo plugin_lang_get( 'mailbox_hostname' ) . ': ' . $t_mailbox_hostname[ 'hostname' ] . ' (' . $t_mailbox_hostname[ 'port' ] . ')<br />';
		echo plugin_lang_get( 'mailbox_encryption' ) . ': ' . $t_mailbox[ 'mailbox_encryption' ] . '<br />';
		echo plugin_lang_get( 'mailbox_username' ) . ': ' . $t_mailbox[ 'mailbox_username' ] . '<br />';
		echo plugin_lang_get( 'mailbox_password' ) . ': ******' . '<br />';
		echo plugin_lang_get( 'mailbox_auth_method' ) . ': ' . $t_mailbox[ 'mailbox_auth_method' ] . '<br />';

		if ( $t_mailbox[ 'mailbox_type' ] === 'IMAP' )
		{
			echo plugin_lang_get( 'mailbox_basefolder' ) . ': ' . $t_mailbox[ 'mailbox_basefolder' ] . '<br />';
		}
